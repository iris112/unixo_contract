<?
	...
	$this->index = function($App, $next) use(&$Db) {
		...
		$fn = function($addr, $deep) use(&$Db, &$users, &$fn) {
			if($deep < 6 && ($ref = $Db->row("SELECT `ref` addr,(SELECT id FROM users WHERE address = t1.ref) id, (SELECT MAX(`level`) FROM sk_events t2 WHERE t2.user = t1.ref AND t2.time > UNIX_TIMESTAMP() - (86400 * 30) AND t2.type = 'UpLevel') level FROM `sk_events` t1 WHERE `user` = ? AND `type` = 'Register'", [$addr]))) {
